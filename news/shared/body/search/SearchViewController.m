...
- (void) httpRequest{
	...
	if (cNetWorkAvailable) {
		...
		[dic setValue:_searchText forKey:@"search"];
...
- (void) filterChanged{
    ...
    [[SQLiteManager sharedInstance] deleteSearch];   //delete old search news
}

- (void) saveToSQLite:(NSArray *)array{
	return;
    //[[SQLiteManager sharedInstance] addSearch:array];
}

- (NSDictionary *)getFromSQLite{
	return nil;
	
    NSDictionary *dic = [[SQLiteManager sharedInstance] getSearchStart:_startIndex limit:cLimit];
    _startIndex += cLimit;
    return dic;
}
